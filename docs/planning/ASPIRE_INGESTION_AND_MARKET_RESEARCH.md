# Aspire baseline: honest ingestion status + market complaint research

**Date:** 2026-07-28  
**Purpose:** Correct the record on how thoroughly Vectric docs were studied, and capture **external** market pain (forums, Reddit, Mac CNC communities, product reviews) so ShopPilot can beat Aspire on *experience*, not only feature count.

---

## Part 1 — Did we ingest the entire Vectric FAQ / User Guide?
...
Vectric’s surface is several things people mix up:

| Source | What it is | Our coverage |
| --- | --- | --- |
| [User Guide hub](https://docs.vectric.com/docs/V12.0/Aspire/ENU/Help/page/user-guide/) | Chapters 01–15 (workflow, 2D, 3D, rotary, posts) | **Read in depth** (including composite/combine modes, rotary origins, post-processor structure outline) |
...
| [What’s New V12](https://docs.vectric.com/docs/V12.0/Aspire/ENU/Help/page/whats-new/) | V12 UI + new tools | **Read** |
| [Vectric support FAQs](https://www.vectric.com/support/faqs/) | Licensing, Mac, installs | **Sampled** (especially Mac) |
...
- **Workflow logic** Vectric teaches: job structure, vectors vs bitmaps, toolpath independence from art + explicit recalculate, preview = trust model, post = translation.
...
5. **Assumptions** in the product plan (stage rail, recipes, Metal heightfield) are **product design**, not claims that Aspire already works that way.

### Baseline integrity rule (going forward)

| Rule | Practice |
| --- | --- |
| **TOC = inventory** | Every form link in the Aspire docs nav gets a row in `FEATURE_PARITY_MATRIX.md` |
| **Form = acceptance criteria** | Before shipping a strategy, engineer opens **that** Vectric form page and maps **every control** to ours or documents intentional omission |
...
- [ ] **DOC-01** Crawl all Aspire form URLs → CSV (name, category, URL).
...
- [ ] **DOC-04** Pair with hardware lab: recreate Aspire tutorial projects as golden jobs.
...
Sources include Vectric’s own Mac FAQ stance, Onefinity/Sienci/Carbide forums, Reddit r/CNC & r/hobbycnc, product comparison articles, YouTube Mac/VM discussions, and public upgrade pricing discussions. Aspire is **widely liked** for 2.5D/artistic CNC; complaints cluster in predictable places. We turn each into a **ShopPilot fix**.

### Complaint cluster 1 — “It doesn’t run on Mac (natively)”

**What people say**

- Vectric’s FAQ: products need **modern Windows**; Mac is via **Parallels/VM**, with **limited support** for that setup.
- Ongoing community content is literally *“How to run Aspire on MacOS”* (Parallels / VMware), not “download the Mac build.”
- Public reports of **VM-specific pain**: dialog crashes on hover/click, **3D view / simulation going black**, version lag until Vectric patches VM issues (e.g. 12.006 called out as fixing Mac VM crashes).
- Sienci and others: “shame there’s no MacOS version”; graphics people live on Mac, CNC software doesn’t.
- Social: direct asks to Vectric “what’s stopping a Mac version?”
...
### Complaint cluster 2 — Price / upgrade ladder / “Aspire is overkill”

**What people say**

- Aspire is repeatedly called **very expensive** / overkill for simple work; VCarve Pro often recommended first.
- Upgrade VCarve Pro → Aspire historically cited around **~$1,300 difference** (full Aspire ~$2k class); annual major upgrades hundreds of dollars for some users.
- Hobbyists ask “how do I afford Aspire?”; many stay on Carbide Create / Fusion free tiers longer than they want.
...
- Feature **gated by SKU** (Desktop vs Pro vs Aspire vs Laser module) creates resentment and decision paralysis.
...
- Reddit: frustration that Aspire/VCarve lack **fully parametric** design — change a dimension and have features/toolpaths update intelligently.
- Comparisons: CAD score lower than Fusion; “horrible at modeling” for full 3D CAD, strong at **2D geometry + fast NC** and **relief**.
- Users bounce Fusion ↔ Vectric: Fusion for parametric assemblies, Vectric for **signs / V-carve / relief speed**.

**Why it hurts**

- Production shops rework jobs (cabinet families, parametric plaques) painfully.
- Guitar / mechanical parts often leave Vectric for Fusion.
...
| **“Linked toolpath” option** (opt-in auto-recalc when source art changes) — default **off** to keep Aspire’s safety model | Medium | Best of both |
...
- Vectric is praised as **easier than Fusion for CNC routers**, but still has **large tool matrices**, design-left / toolpath-right thrash, and many tools never used in a given job.
- V12 explicitly modernized UI (unified panels, DPI, orthographic, view toolbar) — evidence Vectric **knows** density was a problem.
...
- Design-in-Vectric vs design-elsewhere: “not ideal for pure design work.”
...
- Vectric **deliberately** does not auto-update toolpaths when art moves (safety/predictability). Users who don’t know that get **wrong cuts** or confusion (“I moved the text, why didn’t the path move?”).
...
- Aspire designs; **controller software** (Carbide Motion, UGS, machine proprietary) runs the cut. File extension / post mismatches (“.gcode vs .nc”) confuse beginners.
- Mac users juggle **VM Aspire + host serial** awkwardly.
...
| **Pre-flight checklist** (air cut, zero, tool, hold-down) from Vectric’s own run guide | Low | Fewer ruined boards |
...
- Aspire 3D is excellent for **artistic relief / collage components**, not Rhino/Fusion solids.
- Paying Aspire premium **only for 3D creation** while VCarve can *import* 3D frustrates users who need one modeling feature.
...
## Part 3 — Prioritized “beat Aspire” backlog (from market pain)
...
1. **Vectric forum** “Wish List” / “Aspire” subforums — top 50 wishlist threads (need manual browse; search engines under-index).
2. **Facebook CNC groups** Mac + Vectric threads (qualitative).
3. **Support ticket patterns** if any public; YouTube comments on official tutorials.
4. **Win-loss interviews** (5 Aspire users, 5 Mac CNC users on Fusion/Carbide).
5. **Structured survey** after alpha: “What made you leave / avoid Aspire?”
...
2. **Parity matrix** tracks *Aspire capability*; **pain matrix** (this doc) tracks *why users leave*.  
...
| Where must we still match Aspire? | Toolpath quality, V-carve/inlay family, relief modeling depth, posts. |

*Being honest about ingestion is how we get better than Aspire — not by claiming we already are.*
