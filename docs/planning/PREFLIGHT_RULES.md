...
**Source:** reference V12 error strings and validation messages, mapped to actionable preflight checks.
**Verified against:** reference V12 documentation: Vector Validator, Save Toolpaths, V-Carve Toolpath Creator, 2D Profile Toolpath, Toolpath Tabs.
...
|--------|-------|-------------|
| ✅ Verified | 4 | Exact or near-exact reference error string confirmed in V12 docs |
| ⚠️ Partially Verified | 1 | Concept confirmed in the reference but exact error string differs |
| ❌ Unverified | 5 | Concept is sound CAM practice but no reference error string found |

**Key confirmed reference error strings:**
- `"Ignoring unsuitable open vectors"` — official reference error for V-Carve on open vectors (confirmed via the vendor's Instagram/Facebook posts, forum posts, and user-facing messaging)
...
**Key reference features confirmed but without explicit error strings:**
...
| **Reference Equivalent** | **"Ignoring unsuitable open vectors"** ✅ Verified |
...
| **Verification** | ✅ **Verified** — Exact string `"Ignoring unsuitable open vectors"` is the official reference error displayed when V-Carve toolpath encounters open vectors. Confirmed via the vendor's official social media posts, forum posts, and the Vector Validator docs which describe open vector detection. |
...
| **Reference Equivalent** | **Vector Validator marks "intersections"** ✅ Verified (concept); original string `"Self-intersecting path may produce unexpected results"` was approximate |
...
| **Reference Equivalent** | **Vector Validator detects "overlapping contours"** ⚠️ Partially Verified (terminology correction) |
...
| **Verification** | ⚠️ **Partially Verified** — Vector Validator detects "overlapping contours" and the "Overlap Vectors" tool exists for merging overlapping closed vectors. However, the reference's overlap detection is about vectors occupying the same space (which may be intentional, e.g., nested shapes), not about exact duplicate geometry. The original string `"Duplicate geometry detected"` was not found; the reference uses "overlapping contours" / "overlap." The concept is valid but the trigger should be "overlapping contours" not "duplicate geometry." |
...
| **Reference Equivalent** | **No explicit error string found** ❌ Unverified |
...
| **Verification** | ❌ **Unverified** — The V-Carve and Profile forms note that when projecting onto a 3D model, *"its depth is limited so that it does not exceed the bottom of the material."* This is a soft limit (clamping), not an error. The Material Setup form defines stock thickness, but no explicit error string about depth exceeding stock was found. The reference appears to clamp depth rather than error. |
...
| **Reference Equivalent** | **"A different tool number has been defined for each different cutter being used"** ✅ Verified |
...
| **Reference Equivalent** | **Postprocessor ATC config check** ✅ Verified |
