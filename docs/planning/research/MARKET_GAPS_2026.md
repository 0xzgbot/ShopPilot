...
**Purpose:** Polish the ShopPilot feature bar against current competitors and *repeated* operator complaints — not a Vectric form-field crawl.

**Method:** Public forums (Carbide, Sienci, V1E, LightBurn, Sawmill Creek, Vectric FAQ), 2026 comparison writeups, and **YouTube captions ingested this pass** (no video files stored). VectorPilot has **no extra market corpus**; this is ShopPilot research.

**Captions ingested (auto-subs):**

| ID | Title | Why it matters |
| --- | --- | --- |
| [GgeP3nUztpw](https://www.youtube.com/watch?v=GgeP3nUztpw) | 2026 CNC Design Software Review (Scott / Sienci-adjacent) | Explicit CAD vs CAM vs **sender** split; Easel/Carbide/Vectric/Carveco/Fusion/Onshape/gSender ladder |
| [xLPVwGWA8vw](https://www.youtube.com/watch?v=xLPVwGWA8vw) | Vectric UGM 2025 (Edward) | Spark for Mac = **entry** product + V-Transfer GRBL; V13 assembly view; EasyCarve AI/credits; core still Windows |
...
| [IvrXHz2xDUo](https://www.youtube.com/watch?v=IvrXHz2xDUo) | Fusion vs VCarve V-inlay cutting boards | Fusion is free + photoreal; **terrible vector CAM**; VCarve wins join/close/inlay speed |
...
| [vTL6wVD21zk](https://www.youtube.com/watch?v=vTL6wVD21zk) | MillMage vs VCarve welcome sign | LightBurn-shaped design + material wizard vs Vectric |
...
Prior corpus (still valid): 67 Vectric tutorial captions in `extractions/`, `WHAT_IT_TAKES_CNC_APP.md`, `MARKET_RESEARCH.md` Parts 1–5, `USER_WISHLIST_SUMMARY.md`.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Vectric VCarve / Aspire | No (Parallels). Spark = separate entry app | Yes | No (V-Transfer / USB) | Best-in-class | Aspire / Pro 3D | Yes (Pro) | $349–~$2k + upgrades |
| Vectric Spark | Yes (new) | Entry 2D | Yes (V-Transfer built in) | Basic | Not Aspire | Unknown/limited | Entry SKU |
...
| Estlcam | Windows | Fast 2.5D | Some firmwares | Weak vs Vectric | Weak | Simple | ~€49 perpetual |
...
**2026 market move:** LightBurn’s MillMage and Vectric Spark both attack the **same wedge ShopPilot already claimed** (Mac + less Windows tax + some send). Spark is **not** full VCarve. MillMage is the paid product to beat on UX of CAM+control.
...
Scott’s 2026 video still **defines the market as three apps**. Garrett’s gSender video is an entire course because after VCarve/Fusion you still must learn DRO, unlock alarms, jog, probe, macros, start/pause.

Vectric’s own UGM pitch for Spark: “completely self-contained… design all the way through to machining” — they know the split is the beginner killer.
...
Inlay video: VCarve wins because it **rounds corners, adds boundaries, finds zero-span / overlaps**. Fusion is “terrible for editing vectors.”
...
Grayed-out tools until machine+material cutting data exists (Vectric model). Forums still share 40–60 ipm folklore. Nobody wants Fusion’s industrial adaptive defaults on a Shapeoko.
...
Fusion wins customer mockups (colored woods). VCarve preview is trusted for **cut geometry**, not marketing renders.
...
UGM + FAQ: core Vectric remains Windows; Spark is introductory + GRBL. Parallels threads still active in 2026. MillMage still Rosetta.
...
Carbide free: no G-code export (C2D + Motion). Easel: no offline. Vectric: Desktop job-size + 3D gated. Carveco AI: subscription. EasyCreate: **credits**.
...
| Being SolidWorks | 2026 review: Fusion/Onshape for parametric furniture; VCarve for signs. Stay VCarve-shaped. |
...
3. **Vector doctor in Design** (Fusion’s wound, VCarve’s moat).
...
- Scott 2026: “the only wrong software is the one that makes you rage quit”; beginner = Easel/Carbide; signs = Vectric/Carveco; parametric = Fusion/Onshape; **his personal two: VCarve + Onshape**; sender = gSender.
- Vectric UGM: Spark native Mac **entry**; V-Transfer for GRBL/grblHAL (FoxAlien, SainSmart, Sienci-class); V13 assembly; EasyCarve V-text + layouts; **core Windows**.
...
- Fusion inlay: Z0 on spoilboard vs surface; vector hygiene; Fusion photoreal vs VCarve vector CAM.
