...
| 8 | _(vendor form-defaults runspec — removed)_ | Narrow Windows capture: Profile/V-Carve/3D Rough/Finish field defaults | Ready to paste on the Windows PC; merge results into FEATURE_PARITY_MATRIX §R |
...
| 13 | _(vendor form-defaults results — removed)_ | Empty capture tables for the four forms + save summary | human-unblock track (merge → §R) |
...
- Wave-1 competitor research (`WHAT_IT_TAKES_CNC_APP.md`, _(vendor tutorial report — removed)_) remains the core; this wave fills machine/post/preflight/import gaps.
...
| Windows vendor form-defaults capture | needs Windows PC + trial | Paste _(vendor form-defaults runspec — removed)_ on the Windows PC; fill _(vendor form-defaults results — removed)_; merge into §R |
...
Full vendor feature-parity crawls · gadget catalogs · laser · cabinet · cloud tool DB · more YouTube marketing. Wave-2 transcripts only fetched for thin spots (chamfer allowance, bitmap trace — both enriched in `extractions/`); rest of Wave-2 backlog stays queued in _(vendor tutorial catalog — removed)_.
