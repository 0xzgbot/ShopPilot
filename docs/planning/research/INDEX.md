...
| 8 | `VENDOR_FORM_DEFAULTS_RUNSPEC.md` (removed) | Narrow Windows capture: Profile/V-Carve/3D Rough/Finish field defaults | Ready to paste on the Windows PC; merge results into FEATURE_PARITY_MATRIX §R |
...
| 13 | `VENDOR_FORM_DEFAULTS_RESULTS.md` (removed) | Empty capture tables for the four forms + save summary | human-unblock track (merge → §R) |
...
- the incumbent Wave-1 research (`WHAT_IT_TAKES_CNC_APP.md`, `vendor_tutorial_report.md` (removed)) remains the core; this wave fills machine/post/preflight/import gaps.
...
| Windows the incumbent suite form-defaults capture | needs Windows PC + trial | Paste `VENDOR_FORM_DEFAULTS_RUNSPEC.md` (removed) on the Windows PC; fill `VENDOR_FORM_DEFAULTS_RESULTS.md` (removed); merge into §R |
...
Full the incumbent suite feature-parity crawls · gadget catalogs · laser · cabinet · cloud tool DB · more YouTube marketing. Wave-2 transcripts only fetched for thin spots (chamfer allowance, bitmap trace — both enriched in `extractions/`); rest of Wave-2 backlog stays queued in `vendor_tutorial_catalog.csv` (removed).
