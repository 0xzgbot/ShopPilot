# Aspire/VCarve User Wishlist — Top Themes from Forums

**Date:** 2026-07-28  
**Source:** Reddit r/CNC, r/hobbycnc, Vectric forum community discussions.

---

## Theme 1: "Need a Mac-native alternative" (Frequency: Very High)
Users repeatedly express frustration that Aspire/VCarve are Windows-only. Many Mac users resort to Parallels or dual-booting just to use CAM software. This is ShopPilot's primary market wedge — native macOS with Apple Silicon optimization.

## Theme 2: "Subscription pricing is a dealbreaker" (Frequency: High)
Aspire moved to subscription model which alienated many hobbyists. Users want one-time purchase options. ShopPilot's tiered one-time purchase model directly addresses this pain point.

## Theme 3: "Slow preview/rendering" (Frequency: High)
Users complain about Aspire's slow 3D preview and material simulation, especially on large designs. Native Metal-backed rendering in ShopPilot targets this exact complaint.

## Theme 4: "UI is cluttered / too many buttons" (Frequency: Medium-High)
Aspire's interface has large icon matrices that overwhelm new users. The stage rail with ≤12 icons per stage and progressive disclosure directly solves this. Users want tools to appear contextually, not all at once.
...
Aspire sometimes recalculates toolpaths silently after design changes, leading to unexpected results. ShopPilot's dirty flag system with explicit recalculate requirement solves this.

## Theme 8: "Wants better text/lettering tools for signs" (Frequency: Medium-High)
Sign-making is a core Aspire use case. Users want robust text-on-curve, text-to-curves, and V-Carve engraving — all covered in Phase F of ShopPilot's roadmap.
...
Aspire's learning curve is steep. Users want guided workflows, recipe templates (calibration job, sign), and context-sensitive help — all covered by ShopPilot's stage system and coach panel.
...
## Theme 13: "Frustrated by poor customer support / slow Vectric updates" (Frequency: Medium)
Community sentiment is that Vectric moves slowly on feature requests and bug fixes. ShopPilot's open development process with public kanban board addresses this transparency gap.
