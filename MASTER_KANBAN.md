...
- [ ] **SPK-1401h** **MACHINE** (thin, added) Serial write lock — `read()` uses `serialQueue.sync` but `write(_:)` hits FileHandle unqueued; concurrent streamer + status `?` + realtime can race. Files: `RealSerialTransport.swift` only.
  - AC: All port writes (including realtime bytes) go through the same serial queue/lock as configuration. One writer at a time. Do not change GRBL byte meanings. Out of scope: char-count streaming; Hold double-send (1401e done). Verify: `ShopPilotVerify1401e` + `1401f` still PASS; `ShopPilotVerify1401h` asserts/records write serialization.
  - status: claimed by Hermes coder (next slice).
