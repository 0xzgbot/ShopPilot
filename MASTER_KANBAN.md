...
- [x] **SPK-IMPORTBREADTH** **PLAT** Import breadth: OBJ / 3MF / EPS engines + Drill Bank toolpath — **SHIPPED 2026-08-07 (delegated disjoint-file wave + central wiring, Hermes coder)** (SPK-IMPORTBREADTH worklog below)
  - **AC (this slice):** 1) OBJ → heightfield importer (`OBJHeightfieldImporter`, mirrors STL). 2) 3MF → heightfield importer (`ThreeMFImporter`, ZIP+XML). 3) EPS → design vectors (`EPSImporter`, path-operator subset). 4) Drill Bank toolpath (grid W×H, unique hole numbers, through/brad-point, M3 S linkage). All wired: session import methods + ⌘K commands + Design/Model buttons + Cut menu + tree recalc (`.drillBank`). Engine gates CLT-proven: `ShopPilotVerifyOBJImport`, `ShopPilotVerifyEPSImport`, `ShopPilotVerify3MFImport`, `ShopPilotVerifyDrillBank` all PASS. DWG + PDF/AI import NOT in this slice — DWG is a binary format needing its own spec study; PDF is a full document format; deferred with reason (next wave).
  - worklog: 2026-08-07 — Hermes coder. Delegation recipe followed: 4 leaf subagents each owned ONE new engine file + its own CLT (disjoint files, targets pre-registered in Package.swift, wiring centralized here). Landed + independently re-verified: `OBJHeightfield.swift` (ASCII OBJ v/f records, fan triangulation, top-surface rasterize reusing the STL grid math; graceful failures), `EPSImporter.swift` (Geometry: %%BoundingBox offset/scale, moveto/lineto/curveto/closepath → freehand shapes, noise skipping), `ThreeMFImporter.swift` (Process /usr/bin/unzip + XMLParser, vertices/triangles → rasterize). The 4th delegate (Drill Bank) STALLED and left a placeholder — I wrote `DrillBankToolpath.swift` myself: `DrillBankToolpathParams` (gridCols/Rows, spacing, origin, style through/bradPoint, spindleRpm, fromMaterial) + `DrillBankToolpathEngine.compute` (O=DRILL_BANK_TOOLPATH marker, grid header, per-hole numbers, through = −cutDepth, brad-point = −0.8× with comment, M30, time estimate). Central wiring: `Job` untouched (engines are standalone); session `importOBJHeightfield/import3MFHeightfield/importEPSVectors` (+ panel flows + ⌘K importOBJRelief/import3MFRelief/importEPS + Design buttons OBJ Relief…/3MF Relief…/EPS…); `generateDrillBankToolpath/applyDrillBankParams` + `StrategyKind.drillBank` (label detection BEFORE .drill), `drillBankParams()`, recalc branch (grid centered on the selection centroid when closed vectors selected), `ToolFeedApplicable` conformance, Cut menu "Drill Bank". All four CLTs PASS (re-verified by me, not just delegate self-reports); 13-regression sweep green; app debug + release build green. DWG/PDF/AI deferred (binary/format scope).
...
- [ ] **SPK-SHAKE-BUG-ShopPilotVerify0214** **QA** Shakedown failure — `ShopPilotVerify0214`
...
- [ ] **SPK-SHAKE-BUG-ShopPilotVerifyStudio** **QA** Shakedown failure — `ShopPilotVerifyStudio`
