...
- [ ] **SPK-1900e** **CAM** Image-to-Relief — bitmap → heightfield with auto-levels + gaussian smoothing + detail boost (honest name: NOT "AI"). Carveco's marquee feature; our heightfield stack makes this cheap. Files: NEW `Sources/ShopPilotCore/ImageToReliefEngine.swift` + CLT `ShopPilotVerify1900e`; UI = Model-stage "From Image…" beside the component browser. AC: auto-levels stretch CLT-proven on synthetic fixtures, smoothing preserves min/max envelope, params round-trip, node lands in tree + recalc. // engine parallel-ok; wiring serial.
