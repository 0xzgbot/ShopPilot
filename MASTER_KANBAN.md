...
- [x] **SPK-0623b** **QA** Full AX UI drive (every stage + File/Help/Preferences + sheet dismiss)
  ...
  - worklog: 2026-08-13 — Hermes coder, run #3 (driver hardened + binary rebuilt w/ SPK-1700b Play always-visible + SPK-1800a/h AX labels). **FINAL PASS — full AX catalog completed, exit 0.** Walk: Welcome "Try a sample" ok → Setup Advanced open/close ok → Design tools (Select/Rect/Circle/Line/Polyline) + Snap-to-grid + Canvas-origin ok → Cut (Cut out/Pocket/Engrave/More, busy-patience absorbed 35s freeze) ok → Preview Simulate + Play ok → Model Orbit ok → Machine (Simulator→Connect→Idle + Hold/Reset chrome→preflight→Run→Hold→Resume) ok → Preferences open/close ok → Safety Notice (I Understand) ok → preflight→Run→Hold→Resume ok. **No force-quit/dialog-stuck bugs.** New chrome verified live: Snap-to-grid toggle, Canvas origin segmented Picker, Play/Pause always-visible (disabled until sim completes), Orbit checkbox. File/Open/Save skipped (system NSOpenPanel out-of-process). Parent SPK-0623 left [ ]. 
...
# PHASE S — PC-parity wave (SPK-1900) — bring Mac to VectorPilot feature parity + open-source positioning

> Filed 2026-08-21 from the competitive review (VP "mythos" wave H-101…H-210 shipped 2026-08-20/21
> on the owner's PC + 2026 market scan). App is OPEN SOURCE + FREE — positioning vs MillMage ($99)
> and V-carve ($349+) is price + Mac-native + integrated control. Do NOT build payment/licensing UI.
> Engines land as NEW FILES first (parallel-ok), wiring serializes on hot files (AppSession/ContentView).

- [~] **SPK-1900** **PROD** PC-parity parent
  - worklog: 2026-08-21 — Hermes coder. Phase S close-out: 6 of 7 children [x] (a/b/c/d/e/f). Consolidated sweep scripts/sweep_phase_s.sh = 12 PASS / 0 FAIL (4 new CLTs + 4 structural gates + 3 regressions + app build). Parent stays [~] ONLY for 1900g (license choice — owner ON HOLD). — Photo stage + lithophane; Frame + click-to-jog; Beginner/Advanced + starters; machine-dock-everywhere audit; Image-to-Relief; nesting; open-source docs. Deps: none. DoD on parent.
- [x] **SPK-1900a** **CAM** Photo stage + lithophane — photo import → adjust (brightness/contrast/gamma/invert) → heightfield (lithophane thickness mode: light-transmission mapping; grayscale-relief mode: luminance→depth) → three cut paths (lithophane finish-3D / grayscale rough+finish / Photo V-Carve handoff). Concept port of VP H-201/H-210. Files: NEW `Sources/ShopPilotCore/LithophaneEngine.swift` (engine + params, legacy-safe Codable) + CLT `ShopPilotVerify1900a`; wiring (AppSession + stage UI) AFTER engine lands. AC: photo→heightfield grid math CLT-proven (thickness mode inverts luminance, relief mode preserves, both clamp 0…maxDepth), params round-trip + legacy decode, session generate lands node, UI reachable from Design. Verify: `ShopPilotVerify1900a` PASS + `swift_locked.sh build --target ShopPilot`. // engine parallel-ok; wiring serial.
  - worklog: 2026-08-21 — Hermes coder. Engine: LithophaneEngine (thickness/relief modes, tone adjustments, aspect-preserving grid) + ShopPilotVerify1900a PASS (8 assert groups; orchestrator independently re-ran). Wiring: AppSession.generateLithophaneFromPanel() (panel → ImageIO luminance grid → engine → adoptGeneratedRelief lands in job.stlHeightfield, inheriting Rough/Finish 3D + Photo V-Carve + preview + sculpt); Model-stage "Photo Lithophane…" button; ⌘K palette import_lithophane. App target builds. Dedicated Photo STAGE surface deferred to a follow-up card if wanted.
- [x] **SPK-1900b** **MACHINE** Frame job + click-to-jog on canvas — Frame button sends a safe-Z G0 rectangle around job bounds (retract → perimeter at clearance → return); click-to-jog sends G0 to clicked XY when connected. Port of VP H-104. Safety: BOTH paths only when connected AND Idle, reuse the 1401c jog discipline (`\n`-terminated line + G90 restore, single realtime writer untouched); Frame disabled + tooltip when not connected. Files: canvas overlay in `DesignCanvasView.swift`/`ToolpathPreviewView.swift` + thin session methods. AC: frame path = 4 G0s + Z-lifts, no M3, no auto-run; jog click no-ops when disconnected. Verify: `ShopPilotVerify1900b` (formatter math) + python structural gate. // serial (hot canvas files).
  - worklog: 2026-08-21 — Hermes coder. Core: JogToFormatter (absolute G0 XY) + FrameJobFormatter (Z-clearance lift FIRST → 4-move perimeter → origin return; pure G0, no M3/M5, %.3f words). MachineController: canSendMotion gate (connected AND idle) guards frameJob + jogTo — mirrors softHomeHome discipline, realtime writer untouched. DesignCanvas: "Jog-to mode" toggle (tap-only gesture <6px, routes model-space point to jogTo) + "Frame job" button (disabled unless canSendMotion); both AX-labelled with .isButton trait. CLT caught a real engine bug first run (literal X0/Y0 vs %.3f corners) — fixed in formatter. Verify: ShopPilotVerify1900b PASS + scripts/verify_1900b_frame_jog.py PASS (12 checks) + app build.
- [x] **SPK-1900c** **UX** Beginner/Advanced mode + 3 job starters — explicit mode switch (Preferences + Welcome): Beginner hides pro panels beyond the 1400b "Advanced" disclosure (mode gates the SAME panels, never deletes); Advanced = today's app. Starters: Sign (exists), Photo (1900a), 3D relief (exists plaque sample) as one-click recipes. Port of VP H-101. AC: mode persisted (AppSettings), Beginner hides ≥3 pro surfaces, starters each load a full job. Verify: `scripts/verify_1900c_mode.py`. // serial (App.swift + Preferences).
  - worklog: 2026-08-21 — Hermes coder. AppSettings gains persisted beginnerMode (@AppStorage shop_pilot_beginner_mode, default Advanced). Beginner mode: Setup Advanced disclosure fully hidden + collapsed (AX-hidden per BUG-02 discipline, identity preserved for Advanced); ⌘K palette hides pro import formats (DWG/AI/EPS/PDF) via CommandRegistry.beginnerHiddenIDs + mode-aware flatCommands/search; Preferences "Experience mode" picker switches live. Starters: Welcome gains "Start from a Photo…" routing through the real lithophane import; Sign/Plaque samples + New Job + Open intact = 3 starter paths. Nothing deleted — one toggle restores the full app. Verify: scripts/verify_1900c_mode.py PASS + verify_1400b_setup.py regression PASS + app build.
- [x] **SPK-1900d** **UX** Machine dock everywhere audit — VP H-103 gives the PC an app-lifetime machine dock. Mac `machineChrome` exists (ContentView ~:270): AUDIT first — if Hold/Reset/connection state are visible on ALL five stages, card closes with the audit + a screenshot-pack note; if not, extend chrome to persist. AC: audit recorded in worklog with per-stage evidence; safety chrome never hidden while connected (AGENTS.md §2.1). Verify: python structural gate or audit-only close. // serial.
  - worklog: 2026-08-21 — Hermes coder. AUDIT-ONLY CLOSE — the Mac already satisfies VP H-103 parity structurally: TopChromeBar renders unconditionally ABOVE the stage switch (ContentView root body), machineChrome (MachineStatePill + CompactSafetyControls) shows Hold/Resume/Reset whenever chromeState.isLive AND stage != .machine — i.e. on ALL five stages while connected; full-width alarm banner spans every stage; "Safety Req #1" contract documented at the chrome site. No code change needed. Gate: scripts/verify_1900d_dock_audit.py PASS (7 checks).
- [x] **SPK-1900e** **CAM** Image-to-Relief — bitmap → heightfield with auto-levels + gaussian smoothing + detail boost (honest name: NOT "AI"). Carveco's marquee feature; our heightfield stack makes this cheap. Files: NEW `Sources/ShopPilotCore/ImageToReliefEngine.swift` + CLT `ShopPilotVerify1900e`; UI = Model-stage "From Image…" beside the component browser. AC: auto-levels stretch CLT-proven on synthetic fixtures, smoothing preserves min/max envelope, params round-trip, node lands in tree + recalc. // engine parallel-ok; wiring serial.
  - worklog: 2026-08-21 — Hermes coder. Engine: ImageToReliefEngine (robust-percentile auto-levels, separable gaussian, unsharp detail boost, invert) + ShopPilotVerify1900e PASS (8 groups; re-ran). Wiring: AppSession.generateImageToReliefFromPanel() → same relief slot; Model-stage "Image to Relief…" button; ⌘K import_image_to_relief. BitmapHeightfieldImporter.grayscalePixels internal→public (decoder reuse). App target builds.
- [x] **SPK-1900f** **CAM** Nesting — sheet-goods part layout: pack selected vectors/profile parts onto the sheet (shelf/skyline packer, rotation 0/90, spacing + part margin, keep-out zones respected). Known gap on EVERY 2026 reviewer checklist. Files: NEW `Sources/ShopPilotGeometry/NestingEngine.swift` + CLT `ShopPilotVerify1900f` (pure geometry, no session deps); UI = Design-stage "Nest…" later. AC: packer fits N known parts into sheet with ≥spacing, rotation allowed where flagged, deterministic order, overlap-free CLT assertion. // engine parallel-ok; wiring serial.
  - worklog: 2026-08-21 — Hermes coder (partial). Engine: NestingEngine (free-space rect packer + grid variant + contracted AABB skyline packer w/ spacing + rotation) + ShopPilotVerify1900f PASS (7 groups incl overlap-free/spacing/determinism; orchestrator re-ran; delegate TIMED OUT post-landing — verified real per interrupted-delegate protocol). Wiring: AppSession.nestShapesOnSheet(spacingMm:) — group-aware selection or all shapes → skyline packer → translate to slots (translation-only; shape rotation follow-up), undo+dirty+utilization status; Design toolbar Nest button (AX label + .isButton trait, disabled when empty). App target builds. CLOSED: scripts/verify_1900f_nesting.py PASS (20 checks) + regressions 1206/1313/1008/PluginABI PASS + app target build.
- [ ] **SPK-1900g** **DOC** Open-source positioning — LICENSE (owner picks: MIT/Apache-2.0 — ASK, do not choose), README "Free & open source" banner vs paid competitors, CONTRIBUTING.md, repo topic/tags. AC: LICENSE file present (owner-chosen), README states price honestly, no licensing/payment UI anywhere. // parallel-ok, no code.

**Execution order (efficiency plan):**

- worklog: 2026-08-21 — Hermes coder. Wave A DISPATCHED (deleg_1a65af2a): 3 leaf subagents on disjoint new files — 1900a LithophaneEngine+CLT, 1900e ImageToReliefEngine+CLT, 1900f NestingEngine+CLT. Package.swift targets pre-registered + placeholder dirs committed BEFORE dispatch. - worklog: 2026-08-21 — Hermes coder. Wave A ABSORBED: all 3 engines + CLTs landed and independently re-verified by orchestrator — 1900a `ShopPilotVerify1900a: PASS — lithophane engine`, 1900e `ShopPilotVerify1900e: PASS — image-to-relief engine`, 1900f `ShopPilotVerify1900f: PASS — nesting engine`; Core+Geometry+app target builds green; escape-check clean. Note: 1900f delegate TIMED OUT but had landed complete files before stalling (per interrupted-delegate protocol) — verified real, not re-implemented. Engines committed; wiring (Wave B) next, cards reopen on wiring start. License (1900g) ON HOLD per owner.
...
# SPK-1700 — incumbent-style Preview playback (filled heightfield)
...
### 2026-08-26 — PHASE Y audit: differential param reachability (Hermes coder)
- **Motivation:** SPK-2120b shipped `[x]` with a green CLT while the engine cut nothing (3 bugs: param never copied into the engine's params struct; clearance stranded after `M30`; `O=` section emitted with zero `G1`). Marker-presence and JSON-round-trip assertions passed every time, so the same defect class could be hiding elsewhere in PHASE Y.
- **Added `ShopPilotVerifyPhaseYAudit`** (+ `scripts/sweep_phasey.sh`) — asserts on EMITTED CUT MOVES, not markers: `cuttingSections()` attributes every `G1 XY` to its owning `O=` section; `live()` mutates one param and diffs the G-code (byte-identical ⇒ dead param); `programEndSane()` pins exactly one `M30` with only `%` after; semantic direction checks (rest finish cuts less but non-zero, two-pass cuts more than single); a **SELF-CHECK sentinel** proves the audit can fail, so all-green can't just mean an inert harness.
- **Result: 39 assertions, all PASS — no dead params found.** PHASE Y engines (2100a/b/d Rough+Finish 3D, 2110a/b photo, 2120a/b/c V-carve+inlay) are genuinely wired: finish stepOver/toolØ/rasterAngle/previousToolØ, rough stockAllowance/stepDown/inverseMill/previousToolØ, photo tipØ/invert/rasterAngle/twoPass, v-carve tipØ/medialAxisCell/flatAreaClearing all reach the G-code. Byte-stability holds (`previousToolDiameterMm = 0` identical to plain finish). Class-1 stranded-`M30` scan across every Core engine: clean.
- Gates: `ShopPilotVerifyPhaseYAudit` PASS (46/46); phase sweep `scripts/sweep_phasey.sh` **17/17 PASS** (audit, 2100a–d, 2110a/b, 2120a–c, 2010a–c, 3DGolden, Golden25D, VCarveClear, Specialty); `swift_locked.sh build --target ShopPilot` exit 0. Pattern captured in the `shop-pilot-development` skill → `references/toolpath-clt-cut-move-assertions.md`.
- **Gap the audit itself missed (honest note):** the first cut of `inlayFloorGcode` (my SPK-2120b fix) interior-filled EVERY closed path under `vFirst`, which would have pocketed out sign letters. Cursor caught it and added the `inlayInteriorFloor` gate (default OFF; only `computePocket` sets it). My lone-rect fixture could not see it — a sign board needs a letter strictly inside. The audit now guards both directions: a `vFirst` sign board yields V→around-letter clearance with **no cut span entering the letter**, while `inlayInteriorFloor=true` genuinely fills the pocket. Lesson: single-shape fixtures cannot prove protection semantics.
- **Then the guard itself was weak (second self-correction):** the letter check first tested G1 ENDPOINT proximity, which a full-width raster row at the letter's Y (endpoints at x≈1 and x≈29) would sail through. An ad-hoc probe mutation-tested it and confirmed the miss. Replaced with span reconstruction — `cutSegments()` tracks the last commanded XY (G0 *and* G1 move the tool) and `segmentEntersRect()` samples along the span; a synthetic through-cut is asserted to be CAUGHT. Engine verdict unchanged (0 violations across all 9 real clearance segments) — the fix was to the evidence, not the code.
...
- Smoking gun (Sources): `HeightfieldFinishEngine` traces tool-center Z = surface, default `stepOverMm = 0.8` on 3.175 mm ≈ 25% of D (the incumbent suite finish 8–12%); `PhotoVCarveToolpath` stores V-angle but rasters luminance→depth only; `VCarveGeometry` is sharp-point (tip Ø on inlay only).
- Filed PHASE Y: parent SPK-2100 `[ ]` + children 2100a–d, 2110a–b, 2120a–c `[ ]`. Next claim: **2100a**.
- Audit same night: Finish 3D has **no** inspector / `applyFinish3DParams` (Rough 3D does). Split 2100a = engine+CLT only; 2100b = create form + raster angle. Claim-order text that still said “let 2024b finish” was stale — 1920i / 2023a–d / 2024a–c already `[x]`.
...
- [x] **SPK-SHAKE-BUG-ShopPilotVerify1319** **QA** Shakedown failure — `ShopPilotVerify1319`
...
- [x] **SPK-2023b** **GEO** T-bones — bit Ø only prompt; along-X/Y/auto-longest-edge; Dogbone.swift TBone variant; existing dogbone fixtures byte-stable. Test: `ShopPilotVerify2023b`.
  - worklog: 2026-08-25 — Hermes coder (Nous; hit 429 AFTER committing — orchestrator verified + flipped). TBone engine in Geometry/Dogbone.swift beside circular dogbones: square notch per corner, bit Ø only, alongX/alongY/auto-longest-edge; notch reaches exact corner point (bit centerline at exactly r), width = bit Ø. UI: Design-stage corner-relief sheet gains Dogbone/T-bone segmented control + orientation picker; session addTBoneReliefs(bitDiameter:orientation:). Gates re-run by orchestrator: ShopPilotVerify2023b PASS + Verify1301/Verify1313 byte-stable PASS + build --target ShopPilot EXIT=0. Commit 6cb2c82.
...
- [x] **SPK-2024c** **UX** One forward CTA per stage — audit-first (close as audit if already true); coach strip promotes the same action.
  - worklog: 2026-08-25 — Hermes coder (Nous; hit 429 AFTER committing — orchestrator verified + flipped). AUDIT-CLOSE: all five stage-rail stages documented compliant (Design/Cut-forms/Preview/Machine each have exactly one .borderedProminent primary; coach promotions Try a sample / Cut out / Connect match). scripts/verify_2024c_cta.py freezes the audited inventory via brace-counting attribution of every borderedProminent in the app target. No Swift changes needed. Gate re-run by orchestrator: verify_2024c_cta.py PASS. Commit 36a7cda.

Claim order (2026-08-25 night): **SPK-2100a** next (engine-only drop-cutter). PHASE X first-hour/joinery already `[x]` except parked **2022f** and optional **2023e**. Do not reopen medial-axis. Never 0623 / 1900g. Hermes paste: `docs/planning/CUT_QUALITY_HERMES_ORCHESTRATOR_PROMPT.md`.
...
- [x] **SPK-2100** **EPIC** Parent — Cut quality (ball finish + photo grooves + V-bit tip)
  ...
  - Out of scope: Fusion Adaptive 3D, steep/shallow split, pencil, the incumbent suite Offset finish, Voronoi MA rewrite, 2022f, welcome gallery, T-bones
  - Verify: parent `[x]` only when listed children `[x]`
  - deps: none

- [x] **SPK-2100a** **CAM** Drop-cutter / ball compensation on HeightfieldFinishEngine // P0
  - Parent: SPK-2100. Assignee: `coder`. `--max-runtime 90m`. Worktree.
  - AC:
    - ✅ Tool center offset by ball radius R = D/2 (drop-cutter on the heightfield); dome CLT: compensated Z is not surface Z; concave valley not overcut by ~R
    - ✅ Default `stepOverMm = 0.10 * toolDiameterMm` on `init`; **decode** missing key still 0.8 (legacy-safe). New `HeightfieldFinishParams()` jobs get 10% of D.
  - worklog: 2026-08-25 — Hermes coder (kanban subagent). Engine: `HeightfieldFinishEngine.compute` now traces the tool CENTER via grid-sampled drop-cutter (`ballCenterHeight`: max over cell-center samples within d ≤ R of `h + sqrt(R²−d²)`, R = D/2; header comment now says "drop-cutter compensated"). `HeightfieldFinishParams` init takes `stepOverMm: Double? = nil` → defaults to `0.10 * toolDiameterMm` (3.175 mm bit → 0.3175); legacy `init(from:)` decode keeps 0.8 for stored paramsJSON. CLT `ShopPilotVerify2100a` proves: init default 10% of D + explicit wins + missing-key decode 0.8 + round-trip; flat relief → every cut Z = +R ≠ surface Z; dome fixture → apex center ≈ surface+R with a per-move no-gouge sweep; 3 mm groove → centerline lift vs naive trace ≈ R (no ~R overcut). Gate: `./scripts/verify_locked.sh ShopPilotVerify2100a` PASS. Regression sweep: 3DUI/SHAKEf/1920i PASS untouched; 0710 (default stepover), 3Db (peak/corner compensated-Z + positivity bound), 3DGolden (finish golden re-derived by hand under drop-cutter semantics) updated to the verified engine — all PASS. No UI (2100b owns the Finish 3D form).
  - Out of scope: Finish 3D inspector (none exists today — that is **2100b**), Fusion scallop, steep/shallow, pencil, Offset-along-surface
  - Files: `Sources/ShopPilotCore/HeightfieldToolpath.swift` (finish engine + `HeightfieldFinishParams` defaults), new `Sources/ShopPilotVerify2100a/main.swift`, `Package.swift`
...
- [x] **SPK-2100b** **CAM+UI** Create Finish 3D form + raster angle 0/45/90 // P0 · deps: SPK-2100a
  ...
  - Out of scope: the incumbent suite Offset finish with retract; Fusion scallop
