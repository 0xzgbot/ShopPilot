...
final class StatusParserTests: XCTestCase {
    ...
    func testIdleWithMPos() throws {
        let data = Data("<Idle|MPos:0.000,0.000,0.000|FS:0,0>\n".utf8)
        let result = try XCTUnwrap(StatusParser.parse(data))
    ...
    func testRunningWithMPos() throws {
    ...
    func testHoldWithMPosAndWPos() throws {
    ...
    func testFSFeedAndSpindle() throws {
        let data = Data("<Run|MPos:10,20,30|FS:500,1200>\n".utf8)
        let result = try XCTUnwrap(StatusParser.parse(data))
        XCTAssertEqual(result.fs?.feed ?? .nan, 500.0, accuracy: 1e-3)
        XCTAssertEqual(result.fs?.spindle ?? .nan, 1200.0, accuracy: 1e-3)
    }

    func testFSZeroValues() throws {
        let data = Data("<Idle|FS:0,0>\n".utf8)
        let result = try XCTUnwrap(StatusParser.parse(data))
        XCTAssertEqual(result.fs?.feed ?? .nan, 0.0, accuracy: 1e-3)
        XCTAssertEqual(result.fs?.spindle ?? .nan, 0.0, accuracy: 1e-3)
    }

    // MARK: - Bf (buffer) parsing

    func testBufferField() throws {
        let data = Data("<Run|MPos:10,20,30|Bf:15>\n".utf8)
        let result = try XCTUnwrap(StatusParser.parse(data))
        XCTAssertEqual(result.buffer, 15)
    }

    func testBufferZero() throws {
    ...
    func testPinsLimitSwitchTripped() throws {
    ...
    func testPinsProbeTripped() throws {
    ...
    func testPinsControlPins() throws {
    ...
    func testFullReportWithAllFields() throws {
        ...
        XCTAssertEqual(result.fs?.feed ?? .nan, 1000.0, accuracy: 1e-3)
        XCTAssertEqual(result.fs?.spindle ?? .nan, 8000.0, accuracy: 1e-3)
    ...
    func testMinimalReportStateOnly() throws {
        let data = Data("<Idle>\n".utf8)
        let result = try XCTUnwrap(StatusParser.parse(data))
        XCTAssertEqual(result.state, "Idle")
    ...
    func testStateAndMPosOnly() throws {
