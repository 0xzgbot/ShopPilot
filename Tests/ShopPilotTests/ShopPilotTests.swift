...
final class ShopPilotGeometryTests: XCTestCase {
    ...
    func testLineOffsetPositive() {
        ...
        XCTAssertEqual(result?.offsetPath[0].y ?? .nan, 5.0, accuracy: 1e-6)
        XCTAssertEqual(result?.offsetPath[1].y ?? .nan, 5.0, accuracy: 1e-6)
    }
    ...
    func testRectangleOffsetCorners() {
        ...
        XCTAssertEqual(result?.offsetPath[0].x ?? .nan, -2.0, accuracy: 1e-6)
        XCTAssertEqual(result?.offsetPath[0].y ?? .nan, -2.0, accuracy: 1e-6)
    }
    ...
    func testBooleanUnionNonEmpty() {
        ...
        let result = BooleanOps.union(a, b)
        XCTAssertFalse(result.polygons.isEmpty, "Union of two rectangles should not be empty")
    }
    
    func testBooleanIntersectionNonEmpty() {
        let a = VectorShape.rectangle(origin: VectorPoint(x: 0, y: 0), width: 10, height: 10)
        let b = VectorShape.rectangle(origin: VectorPoint(x: 5, y: 0), width: 10, height: 10)
        let result = BooleanOps.intersect(a, b)
        XCTAssertFalse(result.polygons.isEmpty, "Intersection of overlapping rectangles should not be empty")
    }
    ...
    func testBooleanDifferenceNonEmpty() {
        ...
        XCTAssertFalse(result.polygons.isEmpty, "Difference of overlapping rectangles should not be empty")
    }
    ...
    func testExportBlocksMultipleDirtyNodes() {
        ...
        XCTAssertEqual(result.dirtyNodes.count, 2, "Should report both dirty nodes")
    }
    ...
    func testJoinPolylinesEndToEnd() {
        ...
        // b is drawn from 30 back to 10 — its end (10,0) meets a's end (10,0).
        let ptsB: [VectorPoint] = [VectorPoint(x: 30, y: 0), VectorPoint(x: 10, y: 0)]
        let a = VectorShape.freehand(points: ptsA)
        let b = VectorShape.freehand(points: ptsB)
